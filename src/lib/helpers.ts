...
import { CartItemProps, categories, mainCategories, ProductProps, subCategories } from '../types';
 ...
 */
export const isObjLength = (obj: {}) => Boolean(Object.keys(obj).length);
/**
...
export const selectProduct = (products: ProductProps[], id: string): ProductProps | undefined => {
  const product = products.find((el: ProductProps) => el.id === id);
  return product;
};
...
export const extractCategories = (category: string) => {
  ...
  let mainCategory: mainCategories;
  let subCategory: subCategories<keyof mainCategories> | null;
  if (category!.includes('-')) {
    const sliceIndex = category!.indexOf('-');
    mainCategory = category!.slice(0, sliceIndex) as mainCategories
    subCategory = category!.slice(sliceIndex + 1) as subCategories<keyof mainCategories>;
    return { subCategory, mainCategory };
  }
  mainCategory = category as mainCategories;
 ...
 * @param products @type productSlice
 * @param category
 * @returns
 */
export const filterCategories = (products: any, category: string): ProductProps[] => {
...
export const getCartTotal = (cart: CartItemProps[]) => {
...
export const selectRandomProducts = (products: any
  ...
  const isArray = (currentObj: unknown) => currentObj instanceof Array;
  for (let i = 0; i < 6; i++) {
    keys
      .filter(
        (key) =>
 ...
 */
export const toArray = (products: any): ProductProps[] => {
  const productsArr: ProductProps[] = [];
 ...
 */
export const searchProduct = (keyword: string, products: ProductProps[]): ProductProps[] | null => {
...
export const paginateProducts = (products: ProductProps[], pageNum: number) => {
